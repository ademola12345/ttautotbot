...
ctx.replyWithMarkdown(
'🤖 *Stats*\n\n📊 *Total members * *'+dData.length+'*\n*Bot Maker* *@vishu0p* *PM For Making*\n🧭 *Server Time:* *'+time+' IST*')
}}
  catch (err) {
    sendError(err, ctx)
  }
...
if(ctx.message.chat.type != 'private'){
  ...
  let dbData = await db.collection('allUsers').find({userId: ctx.from.id}).toArray()
    ...
    ctx.replyWithMarkdown('💡 *Your WHXC Wallet is:* `'+ dbData[0].coinmail +'`',
   Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change WHXC Wallet', 'iamsetemail')]
      ])
      )  
       .catch((err) => sendError(err, ctx))
    }else{
ctx.replyWithMarkdown('💡 *Your WHXC Wallet is:* _not set_', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change WHXC Wallet', 'iamsetemail')]
      ])
      ) 
...
ctx.replyWithMarkdown("❌ *You have to own at least "+min_wd.toFixed(8)+" "+bot_cur+" in your balance to withdraw!*")
    ...
    ctx.replyWithMarkdown('💡 *Your WHXC Wallet is:* `not set`', 
    Markup.inlineKeyboard([
      [Markup.button.callback('💼 Set or Change YouR WHXC Wallet', 'iamsetemail')]
      ])
      ) 
           .catch((err) => sendError(err, ctx))
...
ctx.replyWithMarkdown("😐 Send a value over *"+min_wd.toFixed(0)+" "+bot_cur+"* but not greater than *"+bal.toFixed(0)+" "+bot_cur+"* ")
return
 }
...
axios
  .post('https://'+hero+'.herokuapp.com/sendThunderToken', 
